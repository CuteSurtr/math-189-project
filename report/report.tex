...
\begin{figure}[H]
    ...
    \caption{Distribution of weekly negative sentiment scores by stratum. Popular games show higher and more variable negativity; volatile games show the lowest mean but with occasional spikes.}
    \label{fig:sentdist}
\end{figure}

\subsection{Time Series Visualization}

Figure~\ref{fig:ts} overlays negative sentiment and log player counts for Counter-Strike~2, ARK: Survival Evolved, and No Man's Sky, chosen as representatives for each game stratum.
...
ARK (decline) shows a gradual downward trend in players; sentiment spikes occasionally coincide with player dips but the relationship is weak.
...
\begin{figure}[H]
    ...
    \caption{Time series: weekly log players (blue, left axis) and negative sentiment (red, right axis) for representative games from each stratum. Sentiment spikes occasionally precede player count dips.}
    \label{fig:ts}
\end{figure}
...
Grand Theft Auto V & Popular & 2 &  6.870 & $0.001$  & *** \\
...
\begin{figure}[H]
    ...
    \caption{LOESS vs.\ OLS linear fit: lagged negative sentiment $\to$ $\Delta\log(\text{players})$, stratified by game stratum. The LOESS curve closely tracks the OLS line, supporting linearity.}
\label{fig:loess}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Out-of-sample predicted vs.\ actual $\log(\text{players})$ on the test set (2023-06-05 to 2024-09-30). RMSE $= 0.509$, only 0.2\% below the na\"ive baseline.}
    \label{fig:oos}
\end{figure}
