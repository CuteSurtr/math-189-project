...
Granger causality tests reject the null of no predictive content for 6 of 15 games ($\alpha = 0.05$), and a two-way fixed effects panel regression yields $\hat{\beta} = -0.474$ ($p = 0.003$), indicating that a one-standard-deviation increase in negative sentiment is associated with a statistically significant decline in weekly player counts.
Bootstrap confidence intervals, permutation tests, LOESS robustness checks, and out-of-sample prediction exercises support the main finding, though power analysis indicates the design is underpowered for detecting small effects.
\end{abstract}
...
\midrule
Popular  & 4 & 12.458 & 0.911 & 0.211 & 0.120 & 365{,}160 \\
Decline  & 5 & 10.794 & 1.147 & 0.167 & 0.071 & 77{,}978 \\
Volatile & 5 & 10.799 & 0.836 & 0.104 & 0.084 & 66{,}262 \\
\midrule
\textbf{Full panel} & \textbf{14} & 11.249 & 1.137 & 0.155 & 0.098 & 142{,}466 \\
\bottomrule
\end{tabular}
\end{table}

The panel contains \textbf{3,457 game-week observations} across 14 games (Counter-Strike 2 was excluded from the final analysis due to a data collection anomaly), with approximately 238--248 weeks per game.
After constructing lag variables and first-differencing, the regression panel has $N = 3{,}840$ rows and 21 columns.
...
Popular games have higher mean negative sentiment ($\bar{S} = 0.211$) and greater variance ($\sigma = 0.120$) than volatile games ($\bar{S} = 0.104$, $\sigma = 0.084$).
...
The correlation between negative sentiment and log players is small but negative ($r \approx -0.08$), consistent with the hypothesized direction.
...
Of the 30 series tested (2 variables $\times$ 15 games), \textbf{29 are stationary at $\alpha = 0.05$}.
The sole exception is Terraria's negative sentiment series (ADF $= -2.47$, $p = 0.12$), which we retain given its borderline nature and the stationarity of its player count series.
...
\midrule
Counter-Strike 2 & log\_players & $-5.595$ & $<0.001$ & Yes \\
Counter-Strike 2 & neg\_sentiment & $-7.206$ & $<0.001$ & Yes \\
Dota 2 & log\_players & $-5.895$ & $<0.001$ & Yes \\
Dota 2 & neg\_sentiment & $-7.832$ & $<0.001$ & Yes \\
PUBG & log\_players & $-5.105$ & $<0.001$ & Yes \\
Rust & log\_players & $-3.720$ & $0.004$ & Yes \\
Terraria & neg\_sentiment & $-2.473$ & $0.122$ & \textbf{No} \\
PAYDAY 2 & neg\_sentiment & $-8.914$ & $<0.001$ & Yes \\
ARK & log\_players & $-4.331$ & $<0.001$ & Yes \\
GTA V & neg\_sentiment & $-6.123$ & $<0.001$ & Yes \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Six of 15 games} show significant Granger causality at $\alpha = 0.05$ for at least one lag:
...
\midrule
PUBG & Popular & 1 & 10.886 & 0.001 & *** \\
PAYDAY 2 & Volatile & 1 & 5.221 & 0.023 & ** \\
Rust & Decline & 1 & 3.288 & 0.071 & * \\
Dota 2 & Popular & 3 & 3.049 & 0.029 & ** \\
GTA V & Popular & 4 & 3.071 & 0.017 & ** \\
ARK & Decline & 4 & 3.810 & 0.005 & *** \\
Terraria & Volatile & 3 & 2.937 & 0.034 & ** \\
\bottomrule
\end{tabular}
\end{table}

The strongest effects appear for PUBG and ARK, both games with highly engaged communities where forum sentiment likely reflects genuine frustrations with game balance or technical issues.
...
\textbf{Two games survive the permutation test} at $\alpha = 0.05$:
\begin{itemize}[nosep]
    \item PUBG: observed $F = 10.886$, permutation $p = 0.001$
    \item PAYDAY 2: observed $F = 5.221$, permutation $p = 0.028$
\end{itemize}
Rust is marginally significant (permutation $p = 0.074$).
...
\begin{figure}[H]
    ...
    \caption{Bootstrap distribution of $\hat{\beta}$ (neg\_sent\_lag1) from 2{,}000 replications. The mean is $-0.516$; the 95\% percentile interval is $[-1.176, 0.129]$.}
    \label{fig:boot}
\end{figure}

The bootstrap results are summarized in Table~\ref{tab:bootstrap}.
The bootstrap 95\% percentile interval $[-1.176, 0.129]$ is slightly narrower than the asymptotic interval $[-1.186, 0.143]$, indicating that the asymptotic standard errors are adequate and slightly conservative.
Note that the bootstrap CI for Model~1 includes zero; significance emerges in Model~2 after absorbing time fixed effects.
...
\midrule
Mean $\hat{\beta}^*$ & $-0.516$ & $-0.521$ \\
SE & $0.335$ & $0.339$ \\
95\% CI & $[-1.176,\; 0.129]$ & $[-1.186,\; 0.143]$ \\
CI Width & $1.305$ & $1.329$ \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
Popular  & $-0.308$ & 0.912 & $-0.338$ & 0.736 \\
Decline  & $-0.563$ & 0.659 & $-0.854$ & 0.393 \\
Volatile & $-0.672$ & 0.428 & $-1.570$ & 0.116 \\
\bottomrule
\end{tabular}
\end{table}

The effect is consistently negative across all three strata, with point estimates ranging from $-0.31$ (popular) to $-0.67$ (volatile).
The effect is strongest for volatile games, where the player base is presumably most sensitive to community sentiment shifts.
However, no individual stratum reaches significance, consistent with the power analysis finding that 4--5 games per stratum provide insufficient power.
...
\begin{figure}[H]
    ...
    \caption{Out-of-sample predicted vs.\ actual $\Delta\log(\text{players})$. The model improvement over the na\"ive baseline is modest (0.3\%).}
    \label{fig:oos}
\end{figure}

The 0.3\% improvement is small, confirming that negative sentiment is a \emph{statistically detectable but practically weak} predictor.
...
The Breusch--Pagan LM test yields $\text{LM} = 5.154$, $p = 0.161$.
We \textbf{fail to reject} the null of homoscedasticity, suggesting the constant-variance assumption is reasonable.
...
\begin{enumerate}
    \item \textbf{Temporal precedence (Granger causality):} Negative sentiment Granger-causes player count changes in 6 of 15 games; 2 survive non-parametric permutation testing at $\alpha = 0.05$.
    ...
    \begin{itemize}[nosep]
        ...
        \item Bootstrap CIs are comparable to asymptotic CIs, validating the inference framework.
    \end{itemize}

    \item \textbf{Practical significance:} The out-of-sample improvement is only 0.3\%, and the design is underpowered (power $= 0.34$).
...
We find statistically significant evidence that higher negative sentiment in Steam reviews precedes short-term declines in active player counts.
The two-way fixed effects panel regression yields $\hat{\beta} = -0.474$ ($p = 0.003$), with the 95\% confidence interval excluding zero.
This result is robust to alternative specifications (multi-lag, LOESS, stratum-specific), and Granger causality tests confirm the temporal precedence for several games.
The cumulative three-lag effect of $-0.761$ suggests sustained negative sentiment is more impactful than transient spikes.

\medskip\noindent\textbf{Limitations.}
\begin{itemize}[nosep]
    \item \textbf{Power}: The design is underpowered for small effects (power $= 0.34$ at the estimated effect size), primarily due to the limited number of cross-sectional units ($N = 14$).
    \item \textbf{Causality}: Granger causality establishes temporal precedence but not a true causal mechanism. Unobserved confounders (e.g., a bad patch simultaneously generating negative reviews and causing player departures) cannot be ruled out without an instrument or natural experiment.
    \item \textbf{Sentiment measurement}: We measure sentiment only from Steam reviews; forum posts, Reddit threads, Twitter/X, and Discord messages may provide additional signal.
    \item \textbf{Serial correlation}: Despite clustered standard errors, the strong residual autocorrelation suggests a richer time series model (e.g., panel VAR) may be more appropriate.
    \item \textbf{Generalizability}: Our sample of 15 games, while stratified, represents a small fraction of the Steam catalog. Results may not generalize to mobile games, console exclusives, or free-to-play titles with different monetization structures.
\end{itemize}
